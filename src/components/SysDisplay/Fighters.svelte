<script lang="ts">
    ...
    afterUpdate(() => {
        ...
        if (fighter.hangar !== undefined) {
            const h = $ship.systems.find(
                (x) => x.name === "hangar" && x.id === fighter.hangar
            );
            if (
                h !== undefined &&
                h.isRack &&
                !fighter.mods.includes("robot")
            ) {
    ...
    let fighterTypes = [...systems.fighterType2Name.entries()].sort((a, b) => {
        return a[1].localeCompare(b[1]);
    });
    let fighterMods = [...systems.fighterMod2Name.entries()].sort((a, b) => {
        return a[1].localeCompare(b[1]);
    });
</script>
...
<div class="field">
    ...
    <div class="control">
        <div class="select" id="hangar">
            <select
                bind:value="{fighter.hangar}"
                on:change="{() => ($ship = $ship)}"
            >
                {#each allHangars as h}
                    <option value="{h.id}"
                        >{assignedHangarIds.includes(h.id)
                            ? "(ASSIGNED) "
                            : ""}{h.id}{h.isRack ? " (robot rack)" : ""}</option
                    >
                {/each}
            </select>
        </div>
