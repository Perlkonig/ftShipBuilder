<script lang="ts">
    ...
    onMount(() => {
        if (svgDisplay !== undefined) {
            ...
            text = text.replace(
                `<svg `,
                `<?xml version="1.0" standalone="no"?><!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd"><svg version="1.1" xmlns="http://www.w3.org/2000/svg" `
            );
            text = text.replace(
                `<defs>`,
                `<defs><st` +
                    `yle type="text/css"><![CDATA[ ${fontZen} ${fontRoboto} text { font-family: "Roboto" } .futureFont { font-family: "Zen Dots" } ]]></style>`
            );
            ...
            if (injectXlink) {
                ...
                text = text.replace(
                    `<defs>`,
                    `<defs><st` +
                        `yle type="text/css"><![CDATA[ @import url('https://fonts.googleapis.com/css2?family=Roboto&family=Zen+Dots&display=swap'); text { font-family: "Roboto"} .futureFont { font-family: "Zen Dots" } .svgInvert { filter: invert(1); } ]]></style>`
                );
            }
            svgDataStr =
...
<div class="hidden">
    <canvas width="{width + 2}" height="{height + 2}" bind:this="{pngCanvas}"
    ></canvas>
</div>
