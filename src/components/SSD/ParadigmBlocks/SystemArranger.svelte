<script lang="ts">
    ...
    import type {
        ILayout as ILayoutSystem,
        IElement,
    } from "@/stores/writeShip";
    ...
    $: if (layout !== undefined) {
            ...
            for (const sys of sysSet) {
                ...
                if (!layout.elements.hasOwnProperty(obj.uid)) {
                    ...
                    layout.elements[obj.uid].height =
                        svg.height * layout.blocks.cellsize;
                }
            }
        }
        ...
        for (const key in layout.elements) {
            if (
                ...
                layout.elements[key].x > block.width
            ) {
                outOfBounds = true;
                break;
            }
            if (
                ...
                layout.elements[key].y > block.height
            ) {
                outOfBounds = true;
                break;
            }
            ...
            if (s.name === "turret") {
                ...
                for (const id of s.weapons) {
                    ...
                    if (obj !== undefined) {
                        ...
                        const wTurret =
                            tObj.glyph().width * layout.blocks.cellsize;
                        const hTurret =
                            tObj.glyph().height * layout.blocks.cellsize;
                        ...
                        const wWeapon =
                            wObj.glyph().width * layout.blocks.cellsize;
                        const hWeapon =
                            wObj.glyph().height * layout.blocks.cellsize;
    ...
    const endDrag = (e: MouseEvent | TouchEvent) => {
        if (dragSelected !== undefined) {
            ...
            if (element !== undefined) {
                ...
                if (newx >= block.width) {
                    newx = block.width - layout.blocks.cellsize;
                }
                if (newy < 0) {
                    newy = 0;
                }
                if (newy >= block.height) {
                    newy = block.height - layout.blocks.cellsize;
                }
                element.x = newx;
                element.y = newy;
                layout = layout;
...
{#if layout !== undefined}
    ...
    {#key layout}
        <div class="ssd">
            <svg
                viewBox="-1 -1 {block.width + 2} {block.height + 2}"
                width="100%"
                height="100%"
...
{#if outOfBounds}
    <div class="field">
        <div class="control">
            <button class="button is-danger" on:click="{correctOutOfBounds}"
                >Correct Overflow</button
            >
        </div>
        <p class="help">
            There are elements outside of the SSD boundaries. Clicking the
            button will pull them onto the canvas.
        </p>
    </div>
{/if}
