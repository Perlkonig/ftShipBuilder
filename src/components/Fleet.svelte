...
`;
    ...
    const dupeShip = (idx: number) => {
        savedFleet.update((val) => ({
            ...
            ships: [
                ...val.ships,
                JSON.parse(JSON.stringify(val.ships[idx])),
    ...
    const delShip = (idx: number) => {
        savedFleet.update((val) => ({
            ...val,
            ships: [...val.ships.slice(0, idx), ...val.ships.slice(idx + 1)],
        }));
    };
...
<div class="content">
    <h1>Fleet Manager</h1>
    <p>
        For best results, export to HTML. Titles will format properly, and there
        won't be any buttons. If any of the ships have a red background, it
        means that the ship is not valid. Load it for details.
    </p>
</div>
<div class="level">
        ...
        <div class="field">
            ...
            <div class="control">
                <input
                    ...
                    on:change="{(e) =>
                        savedFleet.update((v) => ({ ...v, name: fleetName }))}"
                />
            </div>
        </div>
    </div>
    ...
    <div class="level-item">
        <div class="control">
            <button
                ...
                on:click="{() =>
                    savedFleet.update(() => ({ name: fleetName, ships: [] }))}"
                >Clear fleet</button
            >
        </div>
    </div>
        ...
        <div class="control">
            <a
                href="{'data:text/json;charset=utf-8,' +
                    encodeURIComponent(JSON.stringify($savedFleet))}"
                download="{`fleet-${new Date().toISOString()}.json`}"
            >
                <button
    ...
    <div class="level-item">
        <div class="control">
            <button
                class="button is-small"
                on:click="{() => (modalLoadJSON = 'is-active')}"
            ...
            <div class="level">
                <div class="level-item">
                    <a href="#anchorBuilder">
                        <button
                            class="button is-small is-info"
                            on:click="{() => loadShip(idx)}">Load</button
                        >
                    </a>
                </div>
                <div class="level-item">
                    <button
                        class="button is-small is-info"
                        on:click="{() => dupeShip(idx)}">Duplicate</button
                    >
                </div>
                <div class="level-item">
                    <button
                        class="button is-small is-danger"
                        on:click="{() => delShip(idx)}">Delete</button
                    >
                </div>
            </div>
        </div>
            ...
            <div class="field">
                ...
                <div class="control">
                    <input
                        ...
                        class="input"
                        placeholder="Select JSON Fleet File"
                        bind:this="{fileInput}"
