...
<article class="media">
    {#if sys.glyph() !== undefined}
        <figure
            class="media-left"
            style="width: {sys.glyph().width}rem; height: {sys.glyph()
                .height}rem;"
        >
            <svg
    ...
    <div class="media-content">
        ...
        {#if sys.name === "adfc" || sys.name === "fireControl" || sys.name === "sensors"}
            <Advanced prop="{prop}" idx="{idx}" />
        {:else if sys.name === "ecm"}
            <Area prop="{prop}" idx="{idx}" />
        {:else if sys.name === "screen"}
            <Aa prop="{prop}" idx="{idx}" />
            <Level
                prop="{prop}"
                idx="{idx}"
                choices="{[
        ...
        {:else if sys.name === "bay"}
            ...
            />
            <Capacity prop="{prop}" idx="{idx}" min="{1}" />
            <Ratio prop="{prop}" idx="{idx}" />
        ...
        {:else if sys.name.startsWith("spinal") && sys.name !== "spinalNova" && sys.name !== "spinalWave"}
            <Range
                ...
                idx="{idx}"
                choices="{[
                    ['short', 'Short range (24 mu)'],
                    ['medium', 'Medium range (32 mu)'],
                    ['long', 'Long range (48 mu)'],
                ]}"
            />
        {:else if ["beam", "emp", "plasmaCannon", "phaser", "transporter", "needle"].includes(sys.name)}
            <Class prop="{prop}" idx="{idx}" min="{1}" max="{4}" />
            <Arcs prop="{prop}" idx="{idx}" />
        {:else if sys.name === "graser"}
            <Class prop="{prop}" idx="{idx}" min="{1}" max="{4}" />
            <Graser prop="{prop}" idx="{idx}" />
            <Arcs prop="{prop}" idx="{idx}" />
        ...
        {:else if sys.name === "pbl"}
            <Class prop="{prop}" idx="{idx}" min="{1}" max="{6}" />
            <Arcs prop="{prop}" idx="{idx}" maxArcs="{3}" />
        {:else if sys.name === "kgun"}
            <Class prop="{prop}" idx="{idx}" min="{1}" />
            <Modifier
                prop="{prop}"
                idx="{idx}"
                choices="{[
            ...
            />
            <Arcs prop="{prop}" idx="{idx}" minArcs="{1}" />
        {:else if sys.name === "gravitic"}
            <Class prop="{prop}" idx="{idx}" min="{1}" max="{3}" />
            <Arcs prop="{prop}" idx="{idx}" minArcs="{1}" />
        ...
        {:else if sys.name === "fighters"}
            <Fighters prop="{prop}" idx="{idx}" />
        {:else if sys.name === "turret"}
            <Turret prop="{prop}" idx="{idx}" />
        {:else if sys.name === "decoy"}
            <Type
                prop="{prop}"
                idx="{idx}"
                choices="{[
                    ['cruiser', 'Cruiser Decoy'],
                    ['capital', 'Capital Decoy'],
                ]}"
            />
        {/if}
        <MassPts prop="{prop}" idx="{idx}" />
    </div>
    <div class="media-right">
        <button class="delete" on:click="{delSystem}"></button>
    </div>
</article>
